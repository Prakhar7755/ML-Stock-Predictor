...
export default function InsightsPage() {
  ...
  function getValueColorClass(val) {
    ...
    if (
      cleanVal.includes('+') ||
      cleanVal.includes('up') ||
      cleanVal.includes('above') ||
      cleanVal.includes('strong')
    ) {
      ...
      <div className="flex flex-col space-y-2 md:flex-row md:items-center md:justify-between md:space-y-0">
        <div>
          ...
          <p className="mt-2 text-sm text-gray-400">
            Educational stock snapshot analysis powered by Google Gemini, with automatic local
            fallback.
          </p>
        </div>
      </div>
      ...
      {state.loading && (
        <div className="mt-12 flex flex-col items-center justify-center py-12 space-y-4">
          ...
          <p className="text-sm text-gray-400 animate-pulse">
            Running analysis models & compiling insights...
          </p>
        </div>
      )}
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-3 text-sm text-gray-400 uppercase tracking-widest">
                  Analyst Engine:{' '}
                  <span className="text-gray-200 font-mono">{state.data.provider}</span>
                </p>
              </div>
            </div>
          ...
          {insight.metrics && insight.metrics.length > 0 && (
            <div className="grid gap-4 sm:grid-cols-3">
              {insight.metrics.map((metric, i) => (
                <div
                  key={i}
                  className="rounded-xl border border-gray-800 bg-gray-900/40 p-5 flex flex-col justify-between hover:border-gray-700/80 transition"
                >
              ...
              <div className="grid gap-4 md:grid-cols-3">
                {insight.highlights.map((highlight, i) => (
                  <div key={i} className="rounded-xl bg-gray-950/20 border border-gray-800/50 p-4">
                    ...
                    <p
                      className={`text-sm font-semibold mt-1 ${getValueColorClass(highlight.value)}`}
                    >
                      {highlight.value}
                    </p>
                  </div>
            ...
            <div className="rounded-2xl border border-amber-900/30 bg-amber-950/5 p-6">
              ...
              <div className="space-y-4">
                {insight.risks.map((risk, i) => (
                  <div
                    key={i}
                    className="flex gap-3 items-start p-4 rounded-xl bg-amber-950/10 border border-amber-900/20"
                  >
