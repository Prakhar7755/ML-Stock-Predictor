...
export default function AlertsPage() {
  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-10 text-gray-100">
      ...
      <p className="mt-2 text-sm text-gray-400">Store user-specific target-price rules and evaluate them against live prices.</p>

      <form onSubmit={submit} className="mt-8 grid gap-3 rounded-xl border border-gray-800 bg-gray-900 p-5 md:grid-cols-4">
        <input className="input" placeholder="Symbol" value={form.symbol} onChange={(e) => setForm({ ...form, symbol: e.target.value })} />
        <input className="input" type="number" placeholder="Target price" value={form.targetPrice} onChange={(e) => setForm({ ...form, targetPrice: e.target.value })} />
      ...
      <div className="mt-8 grid gap-4">
        {state.data?.alerts?.map((alert) => (
          <div key={alert.id} className="flex items-center justify-between rounded-xl border border-gray-800 bg-gray-900 p-5">
            <div>
              <p className="font-semibold">{alert.symbol}</p>
              <p className="text-sm text-gray-500">
                Alert when {alert.direction} ${Number(alert.targetPrice).toFixed(2)}. Current: {price(alert.currentPrice)}
              </p>
            </div>
