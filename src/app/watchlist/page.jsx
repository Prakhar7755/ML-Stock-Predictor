...
export default function WatchlistPage() {
  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-10 text-gray-100">
      ...
      <p className="mt-2 text-sm text-gray-400">A personalized symbol list with live quote cards.</p>

      <form onSubmit={add} className="mt-8 flex gap-3">
        <input className="input" placeholder="AAPL" value={symbol} onChange={(e) => setSymbol(e.target.value)} />
        ...
        {state.data?.quotes?.map((quote) => (
          <div key={quote.symbol} className="rounded-xl border border-gray-800 bg-gray-900 p-5">
            <div className="flex items-start justify-between gap-4">
              ...
              <button onClick={() => remove(quote.symbol)} className="text-sm text-gray-500 hover:text-red-400">Remove</button>
            </div>
