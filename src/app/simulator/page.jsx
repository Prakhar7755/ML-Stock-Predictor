  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-10 text-gray-100">
      ...
      <p className="mt-2 text-sm text-gray-400">
        A simple order journal that adds practical trading-platform depth.
      </p>
      ...
      {state.data?.summary && (
        <p className="mt-6 text-sm text-gray-400">
          Orders: {state.data.summary.orderCount} | Buy value: $
          {state.data.summary.buyValue.toFixed(2)} | Sell value: $
          {state.data.summary.sellValue.toFixed(2)}
        </p>
      )}
      ...
      <div className="mt-8 grid gap-3">
        {state.data?.orders?.map((order) => (
          <div
            key={order._id}
            className="grid gap-2 rounded-xl border border-gray-800 bg-gray-900 p-4 md:grid-cols-5"
          >
            <p className="font-semibold">{order.symbol}</p>
            <p className={order.side === 'buy' ? 'text-emerald-400' : 'text-red-400'}>
